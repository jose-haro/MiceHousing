...
meta<-read_q2metadata("metadataGFvsConv_nolabcontrols_wcat.txt")
str(meta)
...
ggqqplot(meta$pielou_evenness, title = "Evenness")
...
aov.evenness.facility = aov(pielou_evenness ~ facility, data=meta)
aov.evenness.week = aov(pielou_evenness ~ week, data=meta)
#Call for the summary of that ANOVA, which will include P-values
summary(aov.evenness.facility)
summary(aov.evenness.week)
...
TukeyHSD(aov.evenness.facility)
TukeyHSD(aov.evenness.week)
...
levels(meta$facility)
#Re-order the groups because the default is alphabetical order
meta$facility.ord = factor(meta$facility, c("None", "GF", "Conv"))
levels(meta$facility.ord)

#Plot
boxplot(pielou_evenness ~ facility.ord, data=meta, ylab="Peilou evenness")

evenness_boxplot <- ggplot(meta, aes(facility.ord, pielou_evenness)) + 
  geom_boxplot() + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
...
evenness_summary <- meta %>% # the names of the new data frame and the data frame to be summarised
  group_by(facility.ord) %>%   # the grouping variable
...
evenness_se <- ggplot(evenness_summary, aes(facility.ord, mean_evenness, fill = facility.ord)) + 
  geom_col() + 
...
#####
#Normally distributed for Shannon

aov.shannon.facility = aov(shannon_entropy ~ facility, data=meta)
aov.shannon.week = aov(shannon_entropy ~ week, data=meta)

summary(aov.shannon.facility)
summary(aov.shannon.week)

TukeyHSD(aov.shannon.facility)
TukeyHSD(aov.shannon.week)

#levels(meta$facility)
#meta$facility.ord = factor(meta$facility, c("None", "GF", "Conv"))
#levels(meta$facility.ord)

boxplot(shannon_entropy ~ facility.ord, data=meta, ylab="Shannon")

shannon_boxplot <- ggplot(meta, aes(facility.ord, shannon_entropy)) + 
  geom_boxplot() + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
...
shannon_se <- ggplot(shannon_summary, aes(facility.ord, mean_shannon, fill = facility.ord)) + 
  geom_col() + 
...
kruskal.test(faith_pd ~ facility.ord, data=meta)
...
pairwise.wilcox.test(meta$faith_pd, meta$facility.ord, p.adjust.method="BH")

# Like evenness, we see that pd also increases with age.

#Plot
boxplot(faith_pd ~ facility.ord, data=meta, ylab="Faith phylogenetic diversity")

# or with ggplot2

faith_pd <- ggplot(meta, aes(facility.ord, faith_pd)) + 
  geom_boxplot(aes(color = facility.ord)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
...
faith_pd_se <- ggplot(faith_pd_summary, aes(facility.ord, mean_faith_pd, fill = facility.ord)) + 
  geom_col() + 
...
ggsave("output/Faith_pd_se.png", faith_pd_se, height = 2.5, width = 3)

######
#Non-parametric for Observed Features

kruskal.test(observed_features ~ facility.ord, data=meta)
pairwise.wilcox.test(meta$observed_features, meta$facility.ord, p.adjust.method="BH")
boxplot(observed_features ~ facility.ord, data=meta, ylab="Observed Features diversity")
observed_features <- ggplot(meta, aes(facility.ord, observed_features)) + 
  geom_boxplot(aes(color = facility.ord)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Observed Features Diversity", x = "") 
...
observed_features_se <- ggplot(observed_features_summary, aes(facility.ord, mean_observed_features, fill = facility.ord)) + 
  geom_col() + 
...
ggsave("output/observed_features_se.png", observed_features_se, height = 2.5, width = 3)
