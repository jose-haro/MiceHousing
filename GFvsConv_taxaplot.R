...
#
#metadataGFvsConv_nolabcontrols.txt
...
metadata<-read_q2metadata("metadataGFvsConv_nolabcontrols.txt")
str(metadata)
colnames(metadata)[3] = "facility"
levels(metadata$facility)
metadata$facility.ord = factor(metadata$facility, c("Conv", "GF", "None"))
levels(metadata$facility)
...
physeq <- qza_to_phyloseq(
  features="diversity_results/rarefied_table.qza",
  tree="rooted-tree.qza",
  taxonomy = "taxonomy.qza",
  metadata = "metadataGFvsConv_nolabcontrols.txt"
)
...
my_column <- "facility"  #this is the metadata column that we will use in the taxa barplot
...
for(ml in my_level){
  ...
  physeq_meta_filtered$facility = factor(physeq_meta_filtered$facility, c("Conv", "GF", "None"))
...
diagdds = phyloseq_to_deseq2(physeq_deseq, ~ facility)
...
alpha = 0.05
my_contrast = c("facility", "GF", "Conv") 
#my_contrast = c("facility", "None", "GF") 
#my_contrast = c("facility", "None", "Conv") 
...
#From here check that the taxa level can be changed for Genus, Order, whatever
x = tapply(sigtab$log2FoldChange, sigtab$Phylum, function(x) max(x))
x = sort(x, TRUE)
sigtab$Phylum = factor(as.character(sigtab$Phylum), levels=names(x))
...
ggsave(paste0("output/DESeq2_byPhylum-", my_contrast[2], "-", my_contrast[3], ".png"), DESeq_fig, height = 5, width = 15)
