...
metadata<-read_q2metadata("metadataGFvsConv_nolabcontrols_wcat_nodonor.txt")
str(metadata)
colnames(metadata)[3] = "facility"

dataset <- merge(metadata, dataset, by.x = "SampleID", by.y = 0)
facility<-as.vector(unique(dataset$facility)) #need to choose the column how to subset
datasetn<-dataset
datasetn[datasetn==0]<-NA

#i = 1

summary(metadata$facility)
...
for(i in 1:length(facility)){
  #subset the data for a particular treatment YOU MUST ENTER THE HEADER OF THE COLUMN THAT HAS THE DIFFERENT facility IN THIS CASE “Foaming_Status”
  print(paste("reading ",facility[i],sep=""))
  temp<-subset(dataset, get(my_column)==facility[i])
  tempn<-subset(datasetn, get(my_column)==facility[i])
  print(paste("finished reading ",facility[i],sep=""))
  ...
  if (nrow(merged) > 0) {
    merged$trt<-facility[i]
    final_results<-rbind(final_results, merged)
  }   else {
    print("no correlations for this variable")
  }
  
  print(paste("finished ",facility[i],sep=""))
}

strong_results<-subset(final_results, rho >= 0.95)
...
Conv_ASVs<-subset(dataset, get(my_column)==facility[1])

colnames(Conv_ASVs[1:1101])
head(final_results)
ggplot(Conv_ASVs, aes(x = ASV9, y = ASV54)) +
  geom_point()
...
write.csv(subset(strong_results_taxa, trt=="GF"), "output/strong-results-taxa-GF.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="Conv"), "output/strong-results-taxa-Conv.csv", row.names = F)
